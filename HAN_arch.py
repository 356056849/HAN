...
class DyConv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, padding=0, dilation=1, groups=1, ratio=0.25, bias=True, K=4, temperature=34, init_weight=True):
...
class SpatialAttentionModule(nn.Module):
    def __init__(self, low_channels, high_channels):
        ...
        self.low_bilinear = nn.Upsample(scale_factor=2, mode='bilinear')
...
class ChannelAttentionModule(nn.Module):
    def __init__(self, low_channels, high_channels):
        ...
        self.low_fc1 = nn.Conv2d(low_channels, low_channels // 4, 1)
...
class ResidualHybridAttentionGroup(nn.Module):
    def __init__(self, block_chanels, n_blocks):
        super(ResidualHybridAttentionGroup, self).__init__()
        rhab = []
        for i in range(n_blocks):
            rhab.append(ResidualHybridAttentionBlock(block_chanels))
        self.rhab = nn.Sequential(*rhab)
        # self.out = nn.Conv2d(block_chanels, block_chanels, 3, 1, 1)
...
class HybridAttentionNetwork(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nf=64, ng=4, nb=2, upscale=4):
        ...
        rhag = []
        for i in range(ng):
            rhag.append(ResidualHybridAttentionGroup(nf, nb))
        self.rhag = nn.Sequential(
            *rhag,
            nn.PReLU()
        )
    ...
    def forward(self, x):
        ...
        hybrid_feature = self.rhag(shallow_feature)
        deep_feature = hybrid_feature + shallow_feature
        out = self.final_feature_reconstruction(deep_feature)
        return out

    def update_dyconv_temperature(self):
        for i in range(self.n_group):
            for j in range(self.n_block):
                self.rhag[i].rhab[j].msdb_1to2_1.dyconv.update_temperature()
                self.rhag[i].rhab[j].msdb_1to2_2.dyconv.update_temperature()
                self.rhag[i].rhab[j].msdb_2to4_1.dyconv.update_temperature()
        print('Temperature updates to', self.rhag[0].rhab[0].msdb_2to4_1.dyconv.attention.temperature)
...
if __name__ == '__main__':
    ...
    HAN = HybridAttentionNetwork(in_nc=3, nf=64, ng=4, nb=2).cuda()
    for i in range(1):
        HAN.update_dyconv_temperature()
    y = HAN(img)
    print(y.shape)
